...
import { PUNKT_TYP, TRAKTANDUM_TYP, TYP_BADGE, formatDauer } from '../utils/labels.js'
import { dauerSumme, istAntragPunkt, istBerechtigt, kindTyp, neuesTraktandum, neuesUntertraktandum, personenText } from '../utils/traktanden.js'
...
// Typ (Information / Antrag / Anträge / Pendenz) und geplante Dauer werden hier festgelegt; im Protokoll sind sie fix.
// Ein Punkt vom Typ Antrag ist selbst der Antrag (Titel und Notiz werden im Protokoll zum offenen Antrag, keine Unterpunkte);
// bei «Anträge» ist jeder Unterpunkt ein Antrag. Im Protokoll kommen keine Anträge mehr dazu.
export default {
    ...
    <div class="stack">
      ...
      <section v-for="(t, i) in traktanden" :id="t.id" :key="t.id" class="card" :class="{ editierbar: hauptEditierbar(t) && aktiv !== t.id, editing: aktiv === t.id }" @click.capture="hauptEditierbar(t) && aktiv !== t.id && (aktiv = t.id)">
        <div class="traktandum-kopf">
          ...
          <template v-if="aktiv === t.id">
            ...
            <span class="werkzeuge">
              ...
              <MenuDropdown :text="'Einstellungen: ' + ([PUNKT_TYP[t.typ], formatDauer(t.dauer)].filter(Boolean).join(' · ') || '–')" panel>
                <label class="stack-xs"><span class="label">Typ der Einträge</span>
                  <select v-model="t.typ" class="input" title="Gilt im Protokoll für alles unter diesem Traktandum – «Antrag»: Titel und Notiz werden dort zum offenen Antrag; «Anträge»: jeder Unterpunkt ist ein Antrag">
                    <option v-for="(label, wert) in TRAKTANDUM_TYP" :key="wert" :value="wert" :disabled="wert === 'antrag' && t.untertraktanden.length > 0">{{ label }}</option>
                  </select>
                </label>
        ...
        <div v-if="t.untertraktanden.length" class="eingerueckt">
          <Unterpunkte :liste="t.untertraktanden" :nummer="String(i + 1)" :eltern-typ="kindTyp(t.typ)" :geerbt="hauptEditierbar(t)" :pruefen="pruefen" :aktiv-id="aktiv" klasse="editierbar" :bearbeiten="aktiv === t.id" :personen="personen" :bearbeiter-auswahl="bearbeiterAuswahl" :nur-person="nurPerson" @klick="(u, darf) => subKlick(t, u, darf)" @fertig="aktiv = null" />
        </div>
        <div v-if="aktiv === t.id" class="eingerueckt row mt-2">
          <button v-if="!istAntragPunkt(t)" class="btn btn-ghost" @click="t.untertraktanden.push(neuesUntertraktandum(''))">+ Unterpunkt</button>
  ...
  `,
  data() {
    return { neu: '', TRAKTANDUM_TYP, TYP_BADGE, PUNKT_TYP }
  },
  computed: {
    gesamtDauer() {
