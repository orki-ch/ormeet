...
export default {
    ...
    <div>
      <div v-for="e in eigene" :key="e.id" class="eintrag" :class="'typ-' + e.typ">
        ...
        <div v-if="aktiv === e.id" class="block-soft editing stack-sm">
          <div class="row">
            <select v-model="e.typ" class="input w-sm" @change="typGeaendert(e)">
          ...
          <textarea v-model.trim="e.inhalt" class="input" rows="2" placeholder="Inhalt / Beschreibung"></textarea>
          <div class="row">
            <select v-if="e.typ === 'antrag'" v-model="e.antragStatus" class="input w-sm">
        ...
        <!-- Vorschau -->
        <div v-else :class="nurLesen ? '' : 'vorschau'" @click="!nurLesen && (aktiv = e.id)">
      ...
      <form v-if="formularOffen && !nurLesen" class="row mt-1" @submit.prevent="hinzufuegen">
        <select v-model="neu.typ" class="input w-sm">
          <option v-for="(label, wert) in TYP_LABELS" :key="wert" :value="wert">{{ label }}</option>
        </select>
      ...
      <button v-else-if="!nurLesen" class="btn btn-ghost" style="margin-left: -0.65rem" @click="formularOeffnen">+ Eintrag</button>
    </div>
  `,
  data() {
    return {
  ...
  methods: {
    ...
    typGeaendert(eintrag) {
      if (eintrag.typ === 'antrag' && !eintrag.antragStatus) eintrag.antragStatus = 'offen'
